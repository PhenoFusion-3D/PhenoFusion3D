...
class AnalysisDialog(QDialog):
    def __init__(self, controller, parent=None):
        ...
        self.setWindowTitle('Offline reconstruction and trait validation')
        self.resize(850,760)
        ...
        self.buttons=[]
        self.reconstruction_tab();self.traits_tab();self.comparison_tab();self.leaf_tab()
        row=QHBoxLayout()
    ...
    def finished(self,code,status):
        ...
        for name in ('result/index.html','manual_report.html','validation_report.html','plant_1/traits.json','reference_traits.json','profile.json'):
    ...
    def open_latest(self):
        if self.result_path:QDesktopServices.openUrl(QUrl.fromLocalFile(str(self.result_path)))

    def closeEvent(self,event):
        self.cancel();self.process.waitForFinished(2000);event.accept()
